...
LONG WINAPI CLazyException::ExceptionFilter(__in struct _EXCEPTION_POINTERS* pepinfo)
{
	...
	_stprintf(exinfo, _T("程序遇到未知异常，具体信息如下：异常代码：0x%x 标志：0x%x 指令地址：0x%x"), pepinfo->ExceptionRecord->ExceptionCode, pepinfo->ExceptionRecord->ExceptionFlags, pepinfo->ExceptionRecord->ExceptionAddress);
	//查询地址空间中内存地址的信息
