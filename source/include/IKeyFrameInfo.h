...
interface IKeyFrameInfo : public IUnknown
{
	...
	STDMETHOD (GetKeyFrames) (const GUID* pFormat, REFERENCE_TIME* pKFs, UINT& nKFs /* in, out*/) = 0;//pFormat 为：TIME_FORMAT_MEDIA_TIME， pKFs为关键帧时间戳数组
};
...
{
	STDMETHOD (GetKeyFramesPos) (const GUID* pFormat, REFERENCE_TIME* pKFs, LONGLONG *pKFFP, UINT& nKFs) = 0; // return keyframe and keyframe's file pos
