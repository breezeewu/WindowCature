...
#define AudioSource		L"Audio Hook Source"
...
{
    &CLSID_AUDIO_HOOK_SOURCE,// Filter CLSID
...
CFactoryTemplate g_Templates[1] = 
{
    { 
      AudioSource,               // Name
      &CLSID_AUDIO_HOOK_SOURCE,       // CLSID
      CAudioHookSource::CreateInstance, // Method to create an instance of MyComponent
...
STDAPI RegisterFilters( BOOL bRegister )
{
    ...
    if(bRegister)
    { 
        hr = AMovieSetupRegisterServer(CLSID_AUDIO_HOOK_SOURCE, L"screen-capture-recorder", achFileName, L"Both", L"InprocServer32");
    }
    ...
    if( SUCCEEDED(hr) )
    {
        ...
        if( SUCCEEDED(hr) )
        {
            if(bRegister)
            {
                ...
                hr = fm->RegisterFilter(CLSID_AUDIO_HOOK_SOURCE, L"Audio Hook Source", &pMoniker, &CLSID_AudioInputDeviceCategory, NULL, &rf2);
            }
            else
            {
                hr = fm->UnregisterFilter(&CLSID_AudioInputDeviceCategory, 0, CLSID_AUDIO_HOOK_SOURCE);
            }
        }
    ...
    if( SUCCEEDED(hr) && !bRegister )
        hr = AMovieSetupUnregisterServer( CLSID_AUDIO_HOOK_SOURCE );
