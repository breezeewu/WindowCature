...
STDMETHODIMP CPushPinDesktop::SetVideoHwnd(HWND hwnd, RECT* pRect)
{
	CAutoLock Lock(&m_CritSecLock);

	m_iHwndToTrack = hwnd;
	if(pRect){
		m_rClipRect = m_rScreen = *pRect;
	}
	...
	GetClientRect(m_iHwndToTrack, &m_rScreen);
	
	m_rScreen.right  -= m_rScreen.left; // NB this *fails* for dual monitor support currently... but we just get the wrong width by default, at least with aero windows 7 both can capture both monitors
	m_rScreen.bottom -= m_rScreen.top;
	m_rScreen.left   = m_rScreen.top = 0;
	if(pRect && !IsRectEmpty(pRect))
	{
		//裁剪窗口
		if(pRect->left >= m_rScreen.left && pRect->right <= m_rScreen.right && pRect->right - pRect->left > 0)
		{
			m_rScreen.left = pRect->left;
			m_rScreen.right = pRect->right;
		}

		if(pRect->top >= m_rScreen.top && pRect->bottom <= m_rScreen.bottom && pRect->bottom - pRect->top > 0)
		{
			m_rScreen.top = pRect->top;
			m_rScreen.bottom = pRect->bottom;
		}
	}

	//跨度按8像素对齐，高按2个像素对齐
	int nwidth = m_rScreen.right  - m_rScreen.left;
	int nheight = m_rScreen.bottom - m_rScreen.top;
	nwidth = (nwidth >>3)<<3;
	nheight = (nheight>>1)<<1;
	m_rScreen.right = m_rScreen.left + nwidth;
	m_rScreen.bottom = m_rScreen.top + nheight;

	m_iCaptureConfigWidth = m_rScreen.right - m_rScreen.left;
	m_iCaptureConfigHeight = m_rScreen.bottom - m_rScreen.top;
	if(IsRectEmpty(&m_rOrgRect))
	{
		m_rOrgRect	= m_rScreen;
		/*m_nWidth	= m_iCaptureConfigWidth;
		m_nHeight	= m_iCaptureConfigHeight;*/
		int nHeight = nheight * m_nWidth / nwidth;
		SetVideoSolution(m_nWidth, nHeight);
		//m_nHeight = (m_nHeight >> 1)<<1;
	}
	
	return S_OK;
}

STDMETHODIMP CPushPinDesktop::SetVideoSolution(int nwidth, int nheight)
{
	CheckRange(nwidth, 100, 1280);
	//CheckRange(nheight, 100, 720);

	m_nWidth	= (nwidth>>1)<<1;
	m_nHeight	= (nheight>>1)<<1;

	return S_OK;
}

STDMETHODIMP CPushPinDesktop::GetVideoSolution(int* pnwidth, int* pnheight)
{
	CheckPointer(pnwidth,  E_POINTER);
	CheckPointer(pnheight, E_POINTER);
...
//HRESULT CPushPinDesktop::GetVideoRect(HWND hwnd, RECT* prect)
//{
//	CheckPointer(prect, E_POINTER);
//
//	GetClientRect(hwnd, prect);
//
//	prect->right  -= prect->left; // NB this *fails* for dual monitor support currently... but we just get the wrong width by default, at least with aero windows 7 both can capture both monitors
//	prect->bottom -= prect->top;
//	prect->left   = prect->top = 0;
//	if(!IsRectEmpty(&m_rClipRect))
//	{
//		//裁剪窗口
//		if(m_rClipRect.left >= m_rScreen.left && m_rClipRect.right <= m_rScreen.right && m_rClipRect.right - m_rClipRect.left > 0)
//		{
//			m_rScreen.left = m_rClipRect.left;
//			m_rScreen.right = m_rClipRect.right;
//		}
//
//		if(m_rClipRect.top >= m_rScreen.top && m_rClipRect.bottom <= m_rScreen.bottom && m_rClipRect.bottom - m_rClipRect.top > 0)
//		{
//			m_rScreen.top = m_rClipRect.top;
//			m_rScreen.bottom = m_rClipRect.bottom;
//		}
//
//	}
//
...
//	m_rScreen.bottom = m_rScreen.top + nheight;
//}
STDMETHODIMP CPushPinDesktop::GetFirstBitmapImage(BYTE* pData, int len)
{
	if(pData == NULL || m_pFirstImageData == NULL){
		return E_FAIL;
	}
	if(len >= m_iDataLength)
	{
		memcpy(pData, m_pFirstImageData, m_iDataLength);	
		return S_OK;
	}
...
STDMETHODIMP CPushPinDesktop::GetFirstBitmapinfo(BITMAPINFOHEADER * pBitmapInfo,int *pLen)
{
	if(pBitmapInfo == NULL || pLen == NULL || !m_bFirstData){
		return E_FAIL;
	}
	...
	*pBitmapInfo = pVih->bmiHeader;
	...
	//*pBitmapInfo.biHeight = -tweakableHeader.bmiHeader.biHeight; // prevent upside down conversion from i420...
	pBitmapInfo->biSizeImage = GetBitmapSize(pBitmapInfo);
	*pLen = m_iDataLength;
/*
	///////////////
	BITMAPINFOHEADER BmpHead;
	BITMAPFILEHEADER FileHead;
	int OffBits = sizeof(BITMAPFILEHEADER) + sizeof(BITMAPINFOHEADER);

	FileHead.bfType = ((WORD)('M'<<8)|'B');  
	FileHead.bfSize = OffBits + m_iDataLength;
	FileHead.bfReserved1 = 0;
	FileHead.bfReserved2 = 0;
	FileHead.bfOffBits = OffBits;

	HANDLE handle = CreateFile(L"C://22.bmp"
		,GENERIC_WRITE
		,0
		,NULL
		,CREATE_ALWAYS
	...
	DWORD byteWrited = 0;
	BOOL bWrite = TRUE;
	bWrite &= WriteFile(handle,&FileHead,sizeof(BITMAPFILEHEADER),&byteWrited,NULL);
	bWrite &= WriteFile(handle,pBitmapInfo,sizeof(BITMAPINFOHEADER),&byteWrited,NULL);
	bWrite &= WriteFile(handle,pFirstData,(DWORD)m_iDataLength,&byteWrited,NULL);
	CloseHandle(handle);*/
	return S_OK;
}

STDMETHODIMP CPushSourceDesktop::Pause()
{
...
{   
    ...
    if(riid == _uuidof(IAMStreamConfig))
	 ...
	 else if(riid == IID_IScreenCapture)
        *ppv = (IScreenCapture*)this;
    else
...
STDMETHODIMP_(ULONG) CPushSourceDesktop::AddRef()
{ 
	int ret = GetOwner()->AddRef(); 
	printf("ScreenSource AddRef:%d********\n", ret);
	return ret;
} // gets called often...

STDMETHODIMP_(ULONG) CPushSourceDesktop::Release()
{ 
	int ret = GetOwner()->Release(); 
	printf("ScreenSource Releaseref:%d********\n",ret);
	return ret;
}
...
//void CPushPinDesktop::SetTSDelay(REFERENCE_TIME	rtTSDelay)
//{
//	m_rtTSDelay += rtTSDelay;
//}
