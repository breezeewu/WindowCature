...
// child
class CPushPinDesktop : public CSourceStream, public IAMStreamConfig, public IKsPropertySet, public IScreenCapture
{
...
protected:
	...
	int m_iDataLength;
	REFERENCE_TIME		m_rtTSDelay;
		CCritSec	m_CritSecLock;//ÁÙ½çËø
    //CMediaType m_MediaType;
    //CImageDisplay m_Display;            // Figures out our media type for us
	
	CPushSourceDesktop* m_pParent;
	struct SwsContext *	m_pSwsContext;
	struct AVFrame*		m_pRGB32Frame;
	struct AVFrame*		m_pYV12Frame;
	BYTE*				m_pYV12Data;
	int*				m_nYV12Size;
	HDC hScrDc;
	HBITMAP     hRawBitmap;
...
public:
	...
	//STDMETHODIMP_(ULONG) AddRef();// { int ret = GetOwner()->AddRef(); printf("ScreenSource AddRef:%d********\n",ret);return ret;} // gets called often...
    //STDMETHODIMP_(ULONG) Release();// { int ret = GetOwner()->Release(); printf("ScreenSource Releaseref:%d********\n",ret);return ret;}
	...
	//void SetTSDelay(REFERENCE_TIME	rtTSDelay);
    ...
    //  IScreenCapture
    //////////////////////////////////////////////////////////////////////////
	STDMETHODIMP SetVideoHwnd(HWND hwnd, RECT* prect = NULL);
	STDMETHODIMP SetVideoSolution(int nwidth, int nheight);
	STDMETHODIMP GetVideoSolution(int* pnwidth, int* pnheight);
	STDMETHODIMP GetFirstBitmapImage(BYTE* pData, int len);
	STDMETHODIMP GetFirstBitmapinfo(BITMAPINFOHEADER * pBitmapInfo,int *pLen);

	HRESULT RGB322YV12(BYTE* psrc, int srcwidth, int srcheight, int nsrcpitch, enum PixelFormat srcpixfmt, BYTE* pdst, int dstwidth, int dstheight, int ndstpitch, enum PixelFormat dstpixfmt);
	//HRESULT AVFrameFill(AVFrame* pavframe,BYTE* pbuffer, enum PixelFormat pixfmt, int width,int height);
private:
	void reReadCurrentPosition(int isReRead);
	//HRESULT GetVideoRect(HWND hwnd, RECT* prect);
};
