#include <streams.h>
//#include "..\Include\ffmpeg0.8.4\libswscale\swscale.h"
//#include "..\Include\ffmpeg0.8.4\libavcodec\avcodec.h"
extern "C" {
		...
		m_nHeight(480)
{
	...
	m_rOrgRect.top = m_rOrgRect.left = m_rOrgRect.right = m_rOrgRect.bottom = 0;
...
HRESULT CPushPinDesktop::FillBuffer(IMediaSample *pSample)
{
	...
	while(!gotNew) {
		LocalOutput("CopyScreenToDataBlock begin\n");
      CopyScreenToDataBlock(hScrDc, pData, (BITMAPINFO *) &(pVih->bmiHeader), pSample);
	  LocalOutput("CopyScreenToDataBlock end\n");
...
{   
	...
	if(m_pFirstImageData){
		free(m_pFirstImageData);
		//delete[] m_pFirstImageData;
		m_pFirstImageData = NULL;
	}
...
void CPushPinDesktop::CopyScreenToDataBlock(HDC hScrDC, BYTE *pData, BITMAPINFO *pHeader, IMediaSample *pSample)
{
	...
	int         iFinalStretchHeight = m_rScreen.bottom - m_rScreen.top;//getNegotiatedFinalHeight();
	int         iFinalStretchWidth  = m_rScreen.right - m_rScreen.left;//getNegotiatedFinalWidth();
	...
	//HBITMAP hbmp2 =(HBITMAP)LoadImage(NULL, L"d:\\5.bmp", IMAGE_BITMAP, 0, 0, LR_DEFAULTSIZE|LR_LOADFROMFILE);
	//////HDC bmpdc    = ::CreateCompatibleDC(hdcSrc);
	//::SelectObject(hScrDC, hbmp2);
	//::DrawState(hScrDC,NULL,NULL,(LPARAM)hbmp2,0,0,0,0,0,DST_BITMAP);
	...
	if(m_bConvertToI420) {
	  ...
	  if(!m_bFirstData){
		  m_iDataLength = tweakableHeader.bmiHeader.biSizeImage;
		  m_pFirstImageData = (BYTE*)malloc(m_iDataLength /*+ sizeof(tweakableHeader.bmiHeader)*/);  //new BYTE[m_iDataLength];//
		  //memcpy(m_pFirstImageData, &tweakableHeader.bmiHeader, sizeof(tweakableHeader.bmiHeader));
		  memcpy(m_pFirstImageData/* + sizeof(tweakableHeader.bmiHeader)*/,pOldData,m_iDataLength);
		  m_bFirstData = true;
//#define WRITEIMAGE
#ifdef  WRITEIMAGE
...
#define DibPaletteSize(lpbi)    (DibNumColors(lpbi) * sizeof(RGBQUAD))
		...
		BYTE* pbuffer = m_pFirstImageData;
	 ...
	 // int len;
	  ...
	  RGB322YV12(pOldData, iFinalStretchWidth, iFinalStretchHeight, iFinalStretchWidth * 4, PIX_FMT_BGRA, pData, m_nWidth, m_nHeight, m_nWidth, PIX_FMT_YUV420P);
	} else {
...
int CPushPinDesktop::getCaptureDesiredFinalWidth() {
	...
	//	return m_nWidth;//m_iStretchToThisConfigWidth;
	//} else {
	//	return m_nHeight;//m_iCaptureConfigWidth; // full/config setting, static
	//}
	return m_nWidth;
}
                                      ...
                                      ALLOCATOR_PROPERTIES *pProperties)
{
	...
	if(m_bConvertToI420) {
		int I420size = m_nWidth * m_nHeight * 3/2;
		...
		pProperties->cbBuffer = pProperties->cbBuffer > I420size ? pProperties->cbBuffer : I420size;
	}
...
HRESULT CPushPinDesktop::RGB322YV12(BYTE* psrc, int srcwidth, int srcheight, int nsrcpitch, PixelFormat srcpixfmt, BYTE* pdst, int dstwidth, int dstheight, int ndstpitch, PixelFormat dstpixfmt)
{
	CAutoLock Lock(&m_CritSecLock);
	if(NULL == m_pSwsContext)
	{
		m_pSwsContext = sws_getContext(srcwidth,									//源视频宽
										srcheight,									//源视频高
											srcpixfmt,								//源视频数据格式
												dstwidth,							//目标视频宽
													dstheight,						//目标视频高
	...
	CheckPointer(m_pSwsContext, E_FAIL);
	CheckPointer(psrc,			E_POINTER);
	CheckPointer(pdst,			E_POINTER);

	//if(nullptr == m_pYV12Data)
	//{
	//	//分配yv12内存大小
	//	m_nYV12Size = avpicture_get_size(PIX_FMT_YUV420P, ndstpitch, dstheight);
	//	m_pYV12Data = (BYTE*)av_malloc(m_nYV12Size);
	//	memset(m_pYV12Data, 0, m_nYV12Size);
	//}
	//if(nullptr  == m_pYV12Frame)
	//{
	//	m_pYV12Frame = avcodec_alloc_frame();
	//}
	/*int numBytes = 0;
	pSample->actualsamplesize = numBytes = avpicture_get_size(PIX_FMT_BGRA,pFrame->width,pFrame->height);
	avpicture_fill((AVPicture*)m_pRGB32Frame,buffer,PIX_FMT_BGRA,pFrame->width,pFrame->height);*/

	uint8_t* dstdata[] = 
	{
		pdst,
		pdst + ndstpitch * dstheight * 5 /4,
		pdst + ndstpitch * dstheight
	};
	...
	//ffmpeg自带的色彩空间转换，默认使用的是C语言版
	sws_scale(m_pSwsContext,srcdata,srclinesize,0,srcheight,dstdata,dstlinesize);
	return S_OK;
}

//HRESULT CPushPinDesktop::AVFrameFill(AVFrame* pavframe,BYTE* pbuffer,PixelFormat pixfmt, int width,int height)
//{
//	//检测输入参数的合法性
//	CheckPointer(pavframe, E_POINTER);
//	CheckPointer(pbuffer, E_POINTER);
//
//	//avpicture_fill((AVPicture*)pavframe, pbuffer, pixfmt, width, height);
//}
