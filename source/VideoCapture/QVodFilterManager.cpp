...
#include "../include/xmlParser/Xmlparse.h"
...
bool QvodFilterManager::ParserXML(BYTE* src)
{
	...
	if (bOK)
	{
		...
		if(xmlRoot.SelectSingleNode_(_T("Filters"), xmlFilters))//QvodFilter
		{
...
bool QvodFilterManager::ReadFromBin()
{
	...
	HRSRC hRsrc = NULL;//FindResource(hMod, MAKEINTRESOURCE(IDR_XML1),L"XML");
	if (NULL == hRsrc)
		return false;
	...
	for(long i=0; i<count; i++)
	{
		...
		else if(name == "Video")
		{
			...
			for(long j=0; j<count; j++)
			{
				...
				{
					CXMLElement decoder;
					if(!viewlist3.GetItemDisp_(k, decoder.m_Node))
					{
						continue;
					}
...
bool QvodFilterManager::GetBestGraph(e_CONTAINER c, e_V_ENCODER ve, e_A_ENCODER ae, std::vector<CLSID>& node) const
{
...
DWORD QvodFilterManager::GetFilterPriority(const CLSID& clsid, QVOD_MEDIA_TYPE& mt) const
{
	...
	for (; pos != mFilters.end(); pos++)
	{
		if (clsid == pos->clsid)
		{
			...
			{
				if (mt.sub == pos->vInTypes[i].sub && mt.major == pos->vInTypes[i].major)
				{
...
void QvodFilterManager::GetFilterByMediaType(DWORD capacity, QVOD_MEDIA_TYPE& type, std::vector<CLSID>& vectClsid) const
{
	...
	for (; it != mFilters.end(); it++)
	{
		if (it->capacity & capacity)//硬件加速解码器也可作为一般解码器使用 chang by zwu
		{
