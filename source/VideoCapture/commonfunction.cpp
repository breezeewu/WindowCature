...
void OutPutInfo(const int level, const wchar_t *pstring, ...)
{
	...
	_Ret = _vswprintf_c_l(info, MAX_PATH, pstring, NULL, _Arglist);
	va_end(_Arglist);
	...
	{
		char Logpath[256];
		int len = GetCurrentDirectoryA(256, Logpath);
		sprintf(&Logpath[len], "\\Log\\");
		len = strlen(Logpath);
		SYSTEMTIME st;
		GetLocalTime(&st);
		sprintf(&Logpath[len],"Media%02u-%02u-%02u %02u-%02u-%02u.txt",  st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond);
		strcpy(m_LogName, Logpath);
		m_pLogFile = fopen(m_LogName, "a+");
		if(m_pLogFile == NULL)
			return;
	}
	char str[256];
	...
	_Ret = _vswprintf_c_l(info, MAX_PATH, pstring, NULL, _Arglist);
	va_end(_Arglist);
	WideCharToMultiByte(CP_ACP, 0, info, wcslen(info) + 1, &str[len], 100, NULL, NULL);
	fwrite(str, strlen(str) + 1, 1, m_pLogFile);
#endif
}